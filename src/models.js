...
let finder = require('fs-finder'),
    ...
    config = _.extend(
        require("../src/config/sequelize.json")[env],
        {
            host: process.env.CASHBACK_DB_HOST || 'localhost',
            database: process.env.CASHBACK_DB_NAME || 'reactdb',
            username: process.env.CASHBACK_DB_USERNAME || 'postgres',
            password: process.env.CASHBACK_DB_PASSWORD || '1',
            dialectOptions: {
                application_name: _.trim(process.env.CASHBACK_DB_APP_NAME)
            },
            replication: {
                write: {
                    username: process.env.CASHBACK_DB_USERNAME,
                    password: process.env.CASHBACK_DB_PASSWORD,
                    host: process.env.CASHBACK_DB_HOST
                },
                read: `${_.trim(process.env.CASHBACK_DB_HOST_REPLICA)},${process.env.CASHBACK_DB_HOST}`.split(',')
